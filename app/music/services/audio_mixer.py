...
возвращает None (caller продолжит с music без vocal).
"""
...
DEFAULT_VOCAL_GAIN_DB = 2.0   # vocal чуть громче на сведении
DEFAULT_MUSIC_GAIN_DB = -3.0  # music чуть тише чтобы голос был разборчив
MAX_DOWNLOAD_MB = 50
...
async def mix_music_and_vocal(
    *,
    ...
    """Смикширует music + vocal и зальёт результат через upload_fn.

    upload_fn: async callable(content: bytes, filename: str, content_type: str) -> str
        (обычно FalProvider.upload_to_storage)

    Возвращает (audio_url, duration_seconds) при успехе, иначе (None, None).
    """
            ...
            with open(dest_path, "wb") as f:
                async for chunk in resp.aiter_bytes(chunk_size=64 * 1024):
                    ...
                    if total > MAX_DOWNLOAD_MB * 1024 * 1024:
                        raise RuntimeError(
                            f"file > {MAX_DOWNLOAD_MB}MB"
                        )
                    f.write(chunk)


async def _ffmpeg_mix(
    music_path: str, vocal_path: str, output_path: str
) -> None:
    # Микшируем 2 input'а:
    # - music: -3dB, чтобы не глушить вокал
    # - vocal: +2dB
    # - amix=inputs=2:duration=longest — длина = max(music, vocal)
    # - normalize=0 чтобы избежать сильной аттенюации в amix
    cmd = [
        "ffmpeg", "-y",
        "-i", music_path,
        "-i", vocal_path,
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"ffmpeg exit={proc.returncode}: {stderr.decode('utf-8', errors='replace')[:500]}"
        )
